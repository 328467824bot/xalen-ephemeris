...
/// Starting degrees (sidereal, 0–360) of all 249 KP sub-divisions.
///
/// The base partition is 27 nakshatras × 9 Vimshottari sub-lords = 243
/// star-lord/sub-lord segments. Krishnamurti's canonical horary table splits
/// any segment that straddles a 30° sign boundary into two separately-numbered
/// sub-divisions (so that every KP number belongs to exactly one sign),
/// yielding 6 extra segments → the well-known 249-division table.
///
/// The returned slice is strictly increasing and has length 249, with
/// `starts[0] == 0.0`. KP number `n` (1-based) covers the half-open arc
/// `[starts[n-1], starts[n])` (the last segment running to 360°).
fn kp_segment_starts() -> &'static [f64] {
    static STARTS: OnceLock<Vec<f64>> = OnceLock::new();
    STARTS.get_or_init(|| {
        // Walk the 243 base sub-segments, recording each segment's [start, end).
        let mut raw: Vec<(f64, f64)> = Vec::with_capacity(243);
        let mut degree = 0.0_f64;
        for nak_idx in 0..27usize {
            let star_lord = Nakshatra::ALL[nak_idx].lord();
            let start_idx = DashaLord::SEQUENCE
                .iter()
                .position(|l| *l == star_lord)
                .unwrap_or(0);
            for sub_i in 0..9usize {
                let dl_idx = (start_idx + sub_i) % 9;
                let span = SUB_SPANS[dl_idx].1;
                let start = degree;
                let end = degree + span;
                raw.push((start, end));
                degree = end;
        ...
        // Split any base segment that crosses a 30° sign boundary so that each
        // KP number lies wholly within one sign.
        const EPS: f64 = 1e-6;
        // Snap a degree that is within EPS of a 30° sign boundary onto the exact
        // boundary. Accumulated sub-span arithmetic leaves boundary-coincident
        // starts a hair below the true boundary (e.g. 179.99999999999994 for
        // 180°); without snapping, dividing by 30 would report the wrong sign.
        let snap = |x: f64| -> f64 {
            let nearest_boundary = (x / 30.0).round() * 30.0;
            if (x - nearest_boundary).abs() < EPS {
                nearest_boundary
            } else {
                x
            }
        };
        let mut starts: Vec<f64> = Vec::with_capacity(249);
        for &(s, e) in &raw {
            let s = snap(s);
            starts.push(s);
            // Internal sign boundaries strictly inside (s, e): 30, 60, … 330.
            let mut b = ((s / 30.0).floor() as i64 + 1) * 30;
            while (b as f64) < e - EPS {
                if (b as f64) > s + EPS {
                    starts.push(b as f64);
                }
                b += 30;
            }
        }
...
fn compute_kp_number(lon: f64) -> u16 {
    ...
    let starts = kp_segment_starts();
    // KP number = count of segment-starts at or before `lon` (1-based).
    // `partition_point` returns the index of the first start strictly greater
    // than `lon`, which equals that count.
    let n = starts.partition_point(|&start| start <= lon);
    n.clamp(1, 249) as u16
}
...
///
/// Returns `None` for the modern outer planets (Uranus/Neptune/Pluto), which
/// have no Vimshottari dasha lord — they are NOT silently collapsed to the Sun.
pub fn planet_to_dasha_lord(p: Planet) -> Option<DashaLord> {
    match p {
        Planet::Sun => Some(DashaLord::Sun),
        Planet::Moon => Some(DashaLord::Moon),
        Planet::Mars => Some(DashaLord::Mars),
        Planet::Mercury => Some(DashaLord::Mercury),
        Planet::Jupiter => Some(DashaLord::Jupiter),
        Planet::Venus => Some(DashaLord::Venus),
        Planet::Saturn => Some(DashaLord::Saturn),
        Planet::Rahu | Planet::NorthNode => Some(DashaLord::Rahu),
        Planet::Ketu | Planet::SouthNode => Some(DashaLord::Ketu),
        // Modern outer planets have no Vimshottari dasha lord.
...
///
/// KP hierarchy (per K.S. Krishnamurti):
...
pub fn compute_significators(chart: &KpChart) -> Vec<KpSignificator> {
    ...
    let cusp_owners: Vec<usize> = (0..12)
        .map(|i| (chart.cusps[i].rem_euclid(360.0) / 30.0) as usize)
        .collect();
    ...
    for &planet in &Planet::VEDIC_NINE {
        ...
        for &(other_planet, star_lord) in &planet_star_lords {
            if Some(star_lord) == planet_to_dasha_lord(planet)
        ...
        for (house_idx, owner_opt) in house_owner_planets.iter().enumerate() {
            if let Some(owner_planet) = owner_opt {
                ...
                if let Some(&(_, owner_star)) =
                    ...
                    && Some(owner_star) == planet_to_dasha_lord(planet)
                {
                    let h = house_idx + 1;
...
///
/// For marriage, houses 2, 7, 11 are favorable per KP theory.
...
// ---------------------------------------------------------------------------
// KP Number to Sign mapping
// ---------------------------------------------------------------------------

/// Map a KP number (1-249) to its zodiac **sign** (1=Aries through 12=Pisces).
///
...
///
/// NOTE: this returns the **zodiac sign**, not the astrological **house**.
/// Houses are determined by the birth chart's house cusps (Placidus in KP),
/// which require the ascendant and date/time/place — they are not derivable
/// from a KP number alone. The previous name `kp_number_to_house` was
/// misleading; it always computed the sign.
pub fn kp_number_to_sign(kp_num: u16) -> usize {
    if kp_num == 0 || kp_num > 249 {
        return 1; // guard
    }

    // Inverse lookup on the canonical 249-segment table: the sign is simply the
    // sign that the segment's starting degree falls in. Because the table splits
    // every sign-boundary crossing, the start and end of each segment lie in the
    // same sign, so the start alone determines the sign.
    let starts = kp_segment_starts();
    ((starts[(kp_num - 1) as usize] / 30.0) as usize) + 1
}
    ...
    for sig_type in &[
        ...
        for sig in all_significators {
            for &(h, ref st) in &sig.strength_order {
                if h == house && st == sig_type && !result.iter().any(|(p, _)| *p == sig.planet) {
...
mod tests {
    ...
    #[test]
    fn kp_number_covers_full_249_and_matches_published_boundaries() {
        // The table must be exactly 249 entries, strictly increasing, start at 0.
        let starts = kp_segment_starts();
        assert_eq!(starts.len(), 249, "must be exactly 249 KP sub-divisions");
        assert!((starts[0] - 0.0).abs() < 1e-9, "first segment starts at 0");
        ...
        // Sweep the zodiac at 0.01° resolution: every KP number reported must be
        // in 1..=249, and the FULL range 1..=249 must actually be reachable
        // (the old `.min(249)` cap topped out at 243 and never produced 244-249).
        let mut seen = [false; 250];
        let mut deg = 0.0_f64;
        while deg < 360.0 {
            let kp = kp_position(deg);
            assert!(
                (1..=249).contains(&kp.kp_number),
                "KP number {} out of range at {deg}°",
                kp.kp_number
            );
            seen[kp.kp_number as usize] = true;
            deg += 0.01;
        }
        for n in 1..=249usize {
            assert!(
                seen[n],
                "KP number {n} was never reached over the full zodiac"
            );
        }

        // Spot-check published anchors: KP#1 starts at 0° Aries; KP#249 is the
        // last sub, ending at 360°.
        assert_eq!(compute_kp_number(0.0), 1);
        assert_eq!(compute_kp_number(359.999), 249);
    }
    ...
    fn planet_to_from_dasha_lord_roundtrip() {
        for &p in &Planet::VEDIC_NINE {
            let dl = planet_to_dasha_lord(p).expect("Vedic graha must have a dasha lord");
    ...
    #[test]
    fn outer_planets_have_no_dasha_lord() {
        assert_eq!(planet_to_dasha_lord(Planet::Uranus), None);
        assert_eq!(planet_to_dasha_lord(Planet::Neptune), None);
        assert_eq!(planet_to_dasha_lord(Planet::Pluto), None);
        // The nine Vedic grahas always resolve.
        for &p in &Planet::VEDIC_NINE {
            assert!(planet_to_dasha_lord(p).is_some(), "{p:?} should map");
        }
    }
    ...
    #[test]
    fn kp_number_to_sign_range() {
        for num in 1..=249 {
            let sign = kp_number_to_sign(num);
            assert!(
                sign >= 1 && sign <= 12,
                "KP#{num} mapped to sign {sign}, expected 1-12"
            );
        }
    }

    #[test]
    fn kp_number_to_sign_boundaries() {
        // KP 1 = start of Ashwini = 0 Aries = sign 1
        assert_eq!(kp_number_to_sign(1), 1);
        // KP 249 = last valid sub, deep in Pisces region
        let s249 = kp_number_to_sign(249);
        assert!(
            s249 >= 1 && s249 <= 12,
            "KP 249 should be valid sign, got {s249}"
        );
        // KP subs near the middle should be mid-zodiac signs
        let s125 = kp_number_to_sign(125);
        assert!(
            s125 >= 5 && s125 <= 8,
            "KP 125 should be mid-zodiac sign, got {s125}"
        );
    }

    #[test]
    fn kp_number_to_sign_guard() {
        assert_eq!(kp_number_to_sign(0), 1); // out of range
        assert_eq!(kp_number_to_sign(250), 1); // out of range
    }

    #[test]
    fn kp_number_to_sign_pins_drift_corrected_values() {
        // These numbers exposed the +1-segment drift in the old walk-and-count
        // implementation (it advanced the degree AFTER comparing, so the sign
        // returned belonged to the previous segment for ~27 numbers).
        assert_eq!(kp_number_to_sign(1), 1, "KP#1 = 0° Aries = sign 1");
        assert_eq!(kp_number_to_sign(41), 2, "KP#41 should be sign 2");
        assert_eq!(kp_number_to_sign(143), 7, "KP#143 should be sign 7");
        assert_eq!(kp_number_to_sign(228), 11, "KP#228 should be sign 11");
        assert_eq!(
            kp_number_to_sign(249),
            12,
            "KP#249 = end of Pisces = sign 12"
        );
    }

    #[test]
    fn kp_number_to_sign_is_inverse_of_compute_kp_number() {
        // For any longitude, the sign of its KP number must equal the sign the
        // longitude actually sits in — the two functions share one table.
        let mut deg = 0.0_f64;
        while deg < 360.0 {
            let kp_num = compute_kp_number(deg);
            let mapped_sign = kp_number_to_sign(kp_num);
            let sign = ((deg / 30.0) as usize) + 1;
            assert_eq!(
                mapped_sign, sign,
                "KP#{kp_num} sign {mapped_sign} != sign {sign} at {deg}°"
            );
            deg += 0.05;
        }
    }
