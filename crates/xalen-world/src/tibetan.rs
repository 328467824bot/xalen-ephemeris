...
impl TibetanMonth {
    ...
    /// Return the month from a 1-based number (1..=12), wrapping past 12.
    ///
    /// Input `0` is treated as month 1 rather than underflowing: the previous
    /// `(n - 1)` on a `u32` panicked in debug builds and wrapped to a bogus
    /// month (`u32::MAX % 12`) in release builds. Valid Tibetan month numbers
    /// are 1..=12, so 0 has no meaning and is clamped up to the first month.
    pub fn from_number(n: u32) -> Self {
        // saturating_sub(1) keeps 0 -> 0 (First) instead of underflowing.
        Self::ALL[(n.saturating_sub(1) % 12) as usize]
    }
...
mod tests {
    ...
    #[test]
    fn tibetan_month_from_number_zero_does_not_underflow() {
        // Regression: `from_number(0)` previously panicked in debug (`0u32 - 1`)
        // and wrapped to a garbage month in release. It must now clamp to First.
        assert_eq!(TibetanMonth::from_number(0), TibetanMonth::First);
    }
