...
/// Compute a full Zi Wei Dou Shu chart.
///
/// `lunar_month` (1..=12) and `lunar_day` (1..=30) are clamped into range ONCE
/// here, so every downstream palace uses the same values. Out-of-range input
/// (e.g. from a malformed deserialized payload) yields a clamped, panic-free
/// chart rather than a partial or internally-inconsistent one — month 0 behaves
/// exactly like month 1, month 99 like month 12.
pub fn compute_chart(
    ...
    let lunar_month = lunar_month.clamp(1, 12);
    let lunar_day = lunar_day.clamp(1, 30);
    ...
    let zi_wei_idx = zi_wei_palace(lunar_day, ju);
...
/// Place minor stars based on year stem, year branch, lunar month, and birth hour.
///
...
/// - Zuo Fu (左輔): count FORWARD from Chen (辰, index 4) by lunar month.
///   With m = lunar_month - 1: index = (4 + m) mod 12. Month-only — NOT
///   hour-dependent. (Anchors: month 4 → Wei(7), month 10 → Chou(1).)
/// - You Bi (右弼): count BACKWARD from Xu (戌, index 10) by lunar month.
///   With m = lunar_month - 1: index = (10 - m) mod 12. Month-only — NOT
///   hour-dependent. (Anchors: month 4 → Wei(7), month 10 → Chou(1).)
...
pub fn place_minor_stars(
    ...
    // Lunar month, zero-based, clamped to a valid 1..=12 month.
    let m = (lunar_month.clamp(1, 12) - 1) as usize;
    ...
    // Zuo Fu (左輔): count forward from Chen (4) by lunar month — (4 + m) mod 12.
    // Month-only: independent of birth hour (the previous ming_branch proxy was
    // hour-dependent because Ming Gong is derived from month AND hour).
    let zuo_fu_idx = (4 + m) % 12;
    // You Bi (右弼): count backward from Xu (10) by lunar month — (10 - m) mod 12.
    let you_bi_idx = (10 + 12 - m) % 12;
...
mod tests {
    ...
    #[test]
    fn lunar_month_out_of_range_clamps_consistently() {
        // The `lunar_month` arg became a public `u32`, so invalid months are
        // expressible. They are clamped into 1..=12 (documented, panic-free):
        // month 0 behaves like 1, month 99 like 12 — across the public palace
        // helpers AND the assembled chart, with no internal desync.
        let hour = EarthlyBranch::Zi;
        assert_eq!(ming_gong_branch(0, hour), ming_gong_branch(1, hour));
        assert_eq!(ming_gong_branch(99, hour), ming_gong_branch(12, hour));
        assert_eq!(shen_gong_branch(0, hour), shen_gong_branch(1, hour));
        assert_eq!(shen_gong_branch(99, hour), shen_gong_branch(12, hour));

        let lo = compute_chart(HeavenlyStem::Jia, 0, 0, hour);
        let m1 = compute_chart(HeavenlyStem::Jia, 1, 1, hour);
        assert_eq!(lo.ming_gong_branch, m1.ming_gong_branch);
        assert_eq!(lo.shen_gong_branch, m1.shen_gong_branch);
    }
    ...
    fn zi_wei_palace_day_zero_no_underflow() {
        ...
        for ju in [
            ...
            assert_eq!(
                p, p1,
                "day=0 should produce same result as day=1 for ju={:?}",
                ju
            );
        }
    }
            ...
            for branch in EarthlyBranch::ALL.iter() {
                for month in 1..=12u32 {
                    for hour in EarthlyBranch::ALL.iter() {
                        let stars = place_minor_stars(*stem, *branch, month, *hour);
                        for (star, idx) in &stars {
                            assert!(
                                *idx < 12,
                                "{:?} placed at invalid index {} for {:?}/{:?}/m{}/{:?}",
                                star,
                                idx,
                                stem,
                                branch,
    ...
    #[test]
    fn zuo_fu_you_bi_month_only_and_anchored() {
        // Helper to pull a star's placed branch index.
        fn idx_of(stars: &[(MinorStar, usize)], target: MinorStar) -> usize {
            stars.iter().find(|(s, _)| *s == target).unwrap().1
        }

        // --- Anchors (verified vs AmFengShui classical ZWDS tables) ---
        // Zuo Fu counts forward from Chen(4); You Bi counts backward from Xu(10).
        // Month 4 -> Wei(7) for BOTH; Month 10 -> Chou(1) for BOTH.
        let m4 = place_minor_stars(
            HeavenlyStem::Jia,
            EarthlyBranch::Zi,
            4, // lunar month 4
            EarthlyBranch::Zi,
        );
        assert_eq!(idx_of(&m4, MinorStar::ZuoFu), 7, "Zuo Fu month 4 = Wei(7)");
        assert_eq!(idx_of(&m4, MinorStar::YouBi), 7, "You Bi month 4 = Wei(7)");

        let m10 = place_minor_stars(
            HeavenlyStem::Jia,
            EarthlyBranch::Zi,
            10, // lunar month 10
            EarthlyBranch::Zi,
        ...
        assert_eq!(
            idx_of(&m10, MinorStar::ZuoFu),
            1,
            "Zuo Fu month 10 = Chou(1)"
        );
        assert_eq!(
            idx_of(&m10, MinorStar::YouBi),
            1,
            "You Bi month 10 = Chou(1)"
        );

        // --- Hour-independence: the bug being fixed ---
        // For a FIXED lunar month, Zuo Fu and You Bi must NOT change with the
        // birth hour (previously they were derived from the hour-dependent
        // Ming Gong branch and drifted with the hour).
        for month in 1..=12u32 {
            let mut zuo = None;
            let mut you = None;
            for h in 0..12usize {
                ...
                let z = idx_of(&stars, MinorStar::ZuoFu);
                let y = idx_of(&stars, MinorStar::YouBi);
                assert_eq!(
                    *zuo.get_or_insert(z),
                    z,
                    "Zuo Fu must be hour-independent (month {month})"
                );
                assert_eq!(
                    *you.get_or_insert(y),
                    y,
                    "You Bi must be hour-independent (month {month})"
                );
            }
        }
    }
