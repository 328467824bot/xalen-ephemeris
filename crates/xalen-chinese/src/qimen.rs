...
pub fn compute_qimen(year: i32, month: u32, day: u32, hour: u32) -> QiMenChart {
    ...
    // Determine day and hour stems using the SAME sexagenary
    // calendar as the BaZi day pillar (no fake fixed-30-day-month calendar,
    // which previously produced a wrong, discontinuous day stem across the
    // 366/365-day boundary). `gregorian_to_jd` yields the noon JDN that
    // `sexagenary_day` anchors against, so the day stem is continuous and
    // anchored to 2000-01-01 = Wu-Wu.
    let jd = crate::gregorian_to_jd(year, month, day);
    let day_stem = crate::sexagenary_day(jd).stem;

    // Hour stem derived from the day stem and the Chinese double-hour branch.
