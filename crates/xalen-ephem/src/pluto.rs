...
/// One row of the 43-term Pluto series: (i, j, k, lA, lB, bA, bB, rA, rB) —
/// three integer argument multipliers followed by longitude/latitude/radius
/// sine & cosine coefficients.
type PlutoTerm = (i8, i8, i8, f64, f64, f64, f64, f64, f64);

/// Full 43-term periodic series for Pluto.
///
/// Primary source: Goffin (1989) analytical fit to JPL DE200. The coefficients
/// are reproduced as Meeus Table 37.A (pp. 264-265) in "Astronomical
/// Algorithms" 2nd ed. (secondary source / transcription).
///
...
///
/// Cross-validated against soniakeys/meeus (Go, MIT license) transcription of
/// the same table.
#[rustfmt::skip]
...
/// Analytical Pluto position using the Goffin (1989) DE200 fit. Stated accuracy:
/// ~1 arcminute for 1885-2099. (The committed golden test only asserts a generous
/// 600″/10′ regression ceiling vs DE440 in-window — see
/// `accuracy_vs_de440.rs`; the tighter ~1′ figure is the published model bound,
/// not a separately-asserted golden value.)
///
...
///
/// Primary source: Goffin (1989). Reproduced as Meeus, "Astronomical Algorithms",
/// 2nd ed., Ch. 37, pp. 263-267 (secondary).
pub fn pluto_position(jd_tt: JdTT) -> Result<EclipticPosition, EphemerisError> {
    ...
    // Validity check: the Goffin/Meeus/Steyaert fit (Meeus AA 2nd ed. Ch.37,
    // pp. 263-267) is stated valid for the years 1885 through 2099 inclusive —
    // i.e. through the END of calendar year 2099. `year_approx` is the Julian
    // year measured from J2000.0 (t is in Julian centuries, 1 century = 100
    // Julian years), so the whole of calendar year 2099 spans
    // 2099.0 <= year_approx < 2100.0. The previous `..=2099.0` bound rejected
    // almost the entire calendar year 2099 (everything past its first day),
    // truncating the documented window by up to a full year.
    let year_approx = 2000.0 + t * 100.0;
    if !(1885.0..2100.0).contains(&year_approx) {
    ...
    // The Meeus Pluto series gives heliocentric ecliptic coordinates referred to
    // the (dynamical) mean equinox of J2000. Precession to the equinox of date is
    // applied by the caller (via the rigorous Cartesian rotation) AFTER the
    // light-time iteration — see vsop.rs — so this function returns the raw
    // J2000 position. (Previously it added the scalar general-precession-in-
    // longitude here, which the geocentric path then had to strip back out.)
    Ok(EclipticPosition {
        longitude: lon_rad,
        latitude: lat_rad,
        distance: radius_au,
...
mod tests {
    ...
    fn pluto_meeus_example_37a() {
        // Meeus Example 37.a uses J2000 ecliptic (no precession). `pluto_position`
        // now returns the raw J2000 ecliptic position (precession is applied by
        // the caller after light-time), so we compare directly.
        let jde = JdTT(2448908.5);

        let pos = pluto_position(jde).unwrap();

        let lon_j2000 = pos.longitude.rem_euclid(std::f64::consts::TAU) * RAD_TO_DEG;
    ...
    #[test]
    fn pluto_accepts_all_of_2099_rejects_2100() {
        // ~2099-07-01 is well inside calendar year 2099 and must be ACCEPTED.
        // JD 2487885.5 gives year_approx ~2099.49. The old `..=2099.0` bound
        // wrongly rejected everything past the first day of 2099.
        let jd_mid_2099 = JdTT(2_487_885.5);
        assert!(
            pluto_position(jd_mid_2099).is_ok(),
            "Mid-2099 is inside the documented 1885-2099 window and must be accepted"
        );

        // The very end of calendar year 2099 (just before 2100-01-01) must
        // still be accepted: JD 2488069.5 is 2100-01-01.0 (year_approx ~2099.999),
        // so just inside gives year_approx < 2100.0.
        let jd_end_2099 = JdTT(2_488_069.0);
        assert!(
            pluto_position(jd_end_2099).is_ok(),
            "End of 2099 must be accepted"
        );

        // ~2100-07-01 is outside the window and must be REJECTED.
        // JD 2488251.0 gives year_approx ~2100.50.
        let jd_mid_2100 = JdTT(2_488_251.0);
        assert!(
            pluto_position(jd_mid_2100).is_err(),
            "2100 is outside the 1885-2099 window and must be rejected"
        );
    }
