    ...
    F: Fn(f64) -> f64,
{
    ...
    while jd < jd_end {
        let lo = jd;
        // Clamp the high endpoint to jd_end so a crossing located in the final
        // partial step cannot be refined (and emitted) at a JD beyond the
        // requested range. Without this clamp, `jd += step_days` overshoots
        // jd_end and bisection on [jd-step, jd] can return jd > jd_end.
        jd += step_days;
        let hi = jd.min(jd_end);
        let curr_val = f(hi) - target;

        // Sign change or exact hit = crossing detected
        if prev_val * curr_val < 0.0 {
            if let Some(result) = bisect_crossing(&f, target, lo, hi, 50) {
                results.push(result);
            }
    ...
    F: Fn(f64) -> f64,
{
    ...
    while jd < jd_end {
        let lo = jd;
        // Clamp the high endpoint to jd_end so an ingress located in the final
        // partial step is never refined (and emitted) past the requested range.
        jd += step_days;
        let hi = jd.min(jd_end);
        let curr_lon = longitude_fn(hi);
        ...
        if curr_sign != prev_sign {
            ...
            let refined = bisect_sign_boundary(&longitude_fn, lo, hi, target_lon, 50);
...
mod tests {
    ...
    #[test]
    fn crossing_past_jd_end_is_not_emitted() {
        // f(x) = x crosses 10.5 at x = 10.5, which lies in the final overshoot
        // step (jd 9 -> 12) but BEYOND jd_end = 10.0. The high endpoint must be
        // clamped to jd_end, so this out-of-range crossing must NOT be emitted.
        let results = find_crossing(|x| x, 10.5, 0.0, 10.0, 3.0);
        assert!(
            results.is_empty(),
            "Crossing at 10.5 is past jd_end=10.0 and must not be emitted; got {:?}",
            results.iter().map(|r| r.jd).collect::<Vec<_>>()
        );

        // Every emitted crossing must lie within the requested range.
        let in_range = find_crossing(|x| x, 9.5, 0.0, 10.0, 3.0);
        assert_eq!(in_range.len(), 1, "Crossing at 9.5 is inside the range");
        assert!(
            (in_range[0].jd - 9.5).abs() < 1e-6,
            "Crossing should be at 9.5, got {}",
            in_range[0].jd
        );
        for r in &in_range {
            assert!(
                r.jd <= 10.0 + 1e-9,
                "Emitted crossing {} must not exceed jd_end=10.0",
                r.jd
            );
        }
    }
    ...
    #[test]
    fn sign_ingress_past_jd_end_is_not_emitted() {
        // Longitude increases 1 deg/day starting at 28.0, so it crosses the 30°
        // sign boundary at jd = 2.0. With jd_end = 1.0 and step = 3.0, the
        // overshoot step (jd 0 -> 3) would otherwise detect and refine an
        // ingress at jd=2.0, past jd_end. The clamp to jd_end must suppress it.
        let results = find_sign_ingress(|jd| (28.0 + jd).rem_euclid(360.0), 0.0, 1.0, 3.0);
        assert!(
            results.is_empty(),
            "Ingress at jd=2.0 is past jd_end=1.0 and must not be emitted; got {:?}",
            results.iter().map(|r| r.jd).collect::<Vec<_>>()
        );
    }
