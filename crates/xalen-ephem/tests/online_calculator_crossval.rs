...
fn online_house_cusps_pune_j2000_asc_range() {
    // Pune (18.52N, 73.85E) at noon UT on 2000-01-01 = ~16:55 local solar time
    // (late afternoon). The Sun (~280 deg, Capricorn) is in the western sky,
    // SETTING — so Capricorn is on the DESCENDANT, and the Ascendant (the point
    // rising on the EASTERN horizon) is roughly opposite, in early Cancer.
    // Verified against Swiss Ephemeris (ground truth): ASC = 92.4 deg, DSC = 272.4 deg.
    // (A previous version of this test asserted the Capricorn value — that was
    // the descendant; the ascendant formula had its sign convention flipped.)
    ...
    assert!(
        asc_deg > 80.0 && asc_deg < 110.0,
        "Pune ASC at J2000 noon UT (evening local, Cancer rising): expected ~92 deg, got {:.2} deg",
        asc_deg
    );
}
...
fn sun_sidereal_lahiri_proves_accuracy() {
    ...
    assert_eq!(
        sign_idx, 8,
        "Sun at J2000 should be in Sagittarius (idx 8), got idx {sign_idx}"
    );
}
