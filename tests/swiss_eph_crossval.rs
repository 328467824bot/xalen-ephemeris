...
fn jpl_sun_2024() {
    let lon = almanac()
        .geocentric_longitude_deg(Body::Sun, JdUT1(JD_2024))
        .unwrap();
    let delta = angle_delta(lon, 280.5485);
    assert!(
        delta < 0.001,
        "Sun 2024: {lon:.4}° delta={delta:.4}° ({:.1}\")",
        delta * 3600.0
    );
}
...
fn jpl_moon_2024() {
    let lon = almanac()
        .geocentric_longitude_deg(Body::Moon, JdUT1(JD_2024))
        .unwrap();
    let delta = angle_delta(lon, 161.9070);
    assert!(
        delta < 0.008,
        "Moon 2024: {lon:.4}° delta={delta:.4}° ({:.1}\")",
        delta * 3600.0
    );
}
...
fn jpl_mars_2024() {
    let lon = almanac()
        .geocentric_longitude_deg(Body::Mars, JdUT1(JD_2024))
        .unwrap();
    let delta = angle_delta(lon, 267.6791);
    assert!(
        delta < 0.001,
        "Mars 2024: {lon:.4}° delta={delta:.4}° ({:.1}\")",
        delta * 3600.0
    );
}
...
fn jpl_jupiter_2024() {
    let lon = almanac()
        .geocentric_longitude_deg(Body::Jupiter, JdUT1(JD_2024))
        .unwrap();
    let delta = angle_delta(lon, 35.5844);
    assert!(
        delta < 0.001,
        "Jupiter 2024: {lon:.4}° delta={delta:.4}° ({:.1}\")",
        delta * 3600.0
    );
}
...
fn jpl_sun_1950() {
    let lon = almanac()
        .geocentric_longitude_deg(Body::Sun, JdUT1(JD_1950))
        .unwrap();
    let delta = angle_delta(lon, 280.5144);
    assert!(
        delta < 0.001,
        "Sun 1950: {lon:.4}° delta={delta:.4}° ({:.1}\")",
        delta * 3600.0
    );
}
...
fn jpl_moon_1950() {
    let lon = almanac()
        .geocentric_longitude_deg(Body::Moon, JdUT1(JD_1950))
        .unwrap();
    let delta = angle_delta(lon, 67.5445);
    assert!(
        delta < 0.008,
        "Moon 1950: {lon:.4}° delta={delta:.4}° ({:.1}\")",
        delta * 3600.0
    );
}
...
fn jpl_jupiter_1950() {
    let lon = almanac()
        .geocentric_longitude_deg(Body::Jupiter, JdUT1(JD_1950))
        .unwrap();
    let delta = angle_delta(lon, 306.6180);
    assert!(
        delta < 0.001,
        "Jupiter 1950: {lon:.4}° delta={delta:.4}° ({:.1}\")",
        delta * 3600.0
    );
}
...
fn jpl_sun_2050() {
    let lon = almanac()
        .geocentric_longitude_deg(Body::Sun, JdUT1(JD_2050))
        .unwrap();
    let delta = angle_delta(lon, 281.2579);
    assert!(
        delta < 0.001,
        "Sun 2050: {lon:.4}° delta={delta:.4}° ({:.1}\")",
        delta * 3600.0
    );
}
...
fn jpl_moon_2050() {
    let lon = almanac()
        .geocentric_longitude_deg(Body::Moon, JdUT1(JD_2050))
        .unwrap();
    let delta = angle_delta(lon, 25.3810);
    assert!(
        delta < 0.005,
        "Moon 2050: {lon:.4}° delta={delta:.4}° ({:.1}\")",
        delta * 3600.0
    );
}
...
fn jpl_jupiter_2050() {
    let lon = almanac()
        .geocentric_longitude_deg(Body::Jupiter, JdUT1(JD_2050))
        .unwrap();
    let delta = angle_delta(lon, 121.6322);
    assert!(
        delta < 0.001,
        "Jupiter 2050: {lon:.4}° delta={delta:.4}° ({:.1}\")",
        delta * 3600.0
    );
}
...
fn rahu_ketu_opposite() {
    let a = almanac();
    let rahu = a
        .geocentric_longitude_deg(Body::MeanNode, JdUT1(JD_J2000))
        .unwrap();
...
fn ayanamsa_lahiri_bounds() {
    ...
    assert!(
        lahiri > 23.5 && lahiri < 24.2,
        "Lahiri at J2000: {lahiri:.4}°"
    );
}
