...
> Hora). **Sun + Mercury–Saturn sub-arcsecond** vs DE440; **0 of 5,000,000** charts
> over **0.1°** for any planet/node (1850–2150). Moon ~31″ analytical (truncated ELP),
> ~11″ with the DE440 kernel (the lunar apparent-place reduction, not the kernel, is
> the residual). Pure Rust, zero `unsafe` in the core, thread-safe,
...
- **Velocity / daily motion** for every body, with `is_retrograde()`, and
...
- **NAIF DE440 SPK reader** -- reads real JPL `.bsp` binary files for extended validity range and JPL-grade accuracy; the analytical engine (VSOP87A + ELP2000-82 + IAU 2000B nutation + full IAU 2006/P03 rotation) works standalone with zero data files: **Sun + Mercury–Saturn sub-arcsecond** vs JPL DE440 (Sun 0.21", ≤0.76"), Uranus/Neptune ~1.8–2.5", Moon ~31" (truncated ELP) and Pluto arcminute-class in-window (published Goffin-fit bound ~1′; the committed golden test asserts a 600″/10′ regression ceiling vs DE440 — no test asserts a tighter figure). **Load the DE440 kernel for JPL-grade sub-arcsecond on the Sun + planets and full-range Pluto; the Moon is ~11" via the kernel** (the kernel geometry is exact — the residual is the lunar apparent-place reduction, not the data). Both engines apply the same apparent-place chain (full-rotation precession + nutation + 2-pass annual aberration). Numbers reproducible from this repo via `cargo test -p xalen-ephem --test accuracy_vs_de440` (with a `de440s.bsp` kernel present)
...
| ELP2000-82 (Chapront-Touzé & Chapront; abridged 60+60 terms per Meeus Ch.47) | Moon | ~31" (vs DE440; truncated series -- load DE440 to cut this to ~11") | No (analytical) |
...
| JPL DE440 | Moon | ~11" apparent (raw geometry sub-mas; residual is the lunar apparent-place reduction, not the kernel) | Yes (binary `.bsp`) |
...
work entirely from compiled-in polynomial series. Load DE440 when you need
JPL-grade sub-arcsecond precision on the Moon and outer bodies, full-range Pluto,
or primary directions to the minute. (Eclipse output is global circumstances —
type/magnitude/greatest-eclipse; per-observer contact times are not yet computed.)
...
# Run all tests (1,809 unit + 101 cross-validation pass, 0 fail).
...
- 1,809 unit + 101 cross-validation tests, 0 failures
- 150+ source files
